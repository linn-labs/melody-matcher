...
- Validation/test context windows and negatives are resampled by the dataset. Spearman pools valid predictions across a batch and the loop averages batch metrics; it is not automatically mean per-listener correlation. Rank-percentile scores assign distinct ranks to ties.
